(ns tensorflow.-api.v1.compat.v2.sparse
...
(py/initialize!)
(defonce sparse (import-module "tensorflow._api.v1.compat.v2.sparse"))
(defn add 
  ...
  * `threshold == 0.21`: .1, 0, and -.2 will vanish.
  ...
  "
  [a b  & {:keys [threshold]} ]
    (py/call-attr-kw sparse "add" [a b] {:threshold threshold }))
...
Raises:
  ...
  [axis sp_inputs & {:keys [expand_nonconcat_dims name]
                       :or {name None}} ]
    (py/call-attr-kw sparse "concat" [axis sp_inputs] {:expand_nonconcat_dims expand_nonconcat_dims :name name }))
...
(defn reduce-max 
  ...
  Returns:
    ...
    True.
  "
  [sp_input axis keepdims & {:keys [output_is_sparse name]
                       :or {name None}} ]
    (py/call-attr-kw sparse "reduce_max" [sp_input axis keepdims] {:output_is_sparse output_is_sparse :name name }))
...
(defn reduce-sum 
  ...
  [sp_input axis keepdims & {:keys [output_is_sparse name]
                       :or {name None}} ]
    (py/call-attr-kw sparse "reduce_sum" [sp_input axis keepdims] {:output_is_sparse output_is_sparse :name name }))
...
(defn segment-mean 
  ...
  (py/call-attr sparse "segment_mean"  data indices segment_ids num_segments name ))
...
(defn segment-sqrt-n 
  ...
  "
  [ data indices segment_ids num_segments name ]
  (py/call-attr sparse "segment_sqrt_n"  data indices segment_ids num_segments name ))
...
(defn segment-sum 
  ...
  "
  [ data indices segment_ids num_segments name ]
  (py/call-attr sparse "segment_sum"  data indices segment_ids num_segments name ))
...
(defn split 
  ...
  Raises:
    TypeError: If `sp_input` is not a `SparseTensor`.
  "
  [ sp_input num_split axis name ]
  (py/call-attr sparse "split"  sp_input num_split axis name ))
