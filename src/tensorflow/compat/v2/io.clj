(ns tensorflow.-api.v1.compat.v2.io
...
(defn decode-csv 
  ...
  [records record_defaults & {:keys [field_delim use_quote_delim na_value select_cols name]
                       :or {select_cols None name None}} ]
    (py/call-attr-kw io "decode_csv" [records record_defaults] {:field_delim field_delim :use_quote_delim use_quote_delim :na_value na_value :select_cols select_cols :name name }))
...
(defn decode-raw 
  ...
  "
  [input_bytes out_type & {:keys [little_endian fixed_length name]
                       :or {fixed_length None name None}} ]
    (py/call-attr-kw io "decode_raw" [input_bytes out_type] {:little_endian little_endian :fixed_length fixed_length :name name }))
...
(defn parse-example 
  ...
  (py/call-attr io "parse_example"  serialized features example_names name ))
...
(defn parse-single-example 
  ...
  "
  [ serialized features example_names name ]
  (py/call-attr io "parse_single_example"  serialized features example_names name ))
...
(defn serialize-many-sparse 
  ...
  "
  [sp_input & {:keys [out_type name]
                       :or {name None}} ]
    (py/call-attr-kw io "serialize_many_sparse" [sp_input] {:out_type out_type :name name }))
...
(defn serialize-sparse 
  ...
  [sp_input & {:keys [out_type name]
                       :or {name None}} ]
    (py/call-attr-kw io "serialize_sparse" [sp_input] {:out_type out_type :name name }))
