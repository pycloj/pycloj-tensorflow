(ns tensorflow.-api.v1.compat.v2.distribute.ReductionToOneDevice
...
(defn ReductionToOneDevice 
  "Always do reduction to one device first and then do broadcasting.

    Batch reduction is done by reduction on each element one by one.
  "
  [ reduce_to_device accumulation_fn ]
  (py/call-attr distribute "ReductionToOneDevice"  reduce_to_device accumulation_fn ))
