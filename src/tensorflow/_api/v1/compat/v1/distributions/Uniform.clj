(ns tensorflow.-api.v1.compat.v1.distributions.Uniform
...
(defn Uniform 
  ...
  "
  [ & {:keys [low high validate_args allow_nan_stats name]} ]
   (py/call-attr-kw distributions "Uniform" [] {:low low :high high :validate_args validate_args :allow_nan_stats allow_nan_stats :name name }))
...
(defn range 
  "`high - low`."
  [self   & {:keys [name]} ]
    (py/call-attr-kw self "range" [] {:name name }))
