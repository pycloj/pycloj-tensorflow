...
(py/initialize!)
(defonce ragged (import-module "tensorflow.ragged"))
...
(defn constant-value 
  ...
  Raises:
    ValueError: If the scalar values in `pylist` have inconsistent nesting
      depth; or if ragged_rank or inner_shape are incompatible with `pylist`.
  "
  [pylist dtype ragged_rank inner_shape  & {:keys [row_splits_dtype]} ]
    (py/call-attr-kw ragged "constant_value" [pylist dtype ragged_rank inner_shape] {:row_splits_dtype row_splits_dtype }))
...
(defn placeholder 
  ...
  **Important**: This ragged tensor will produce an error if evaluated.
  ...
  (py/call-attr ragged "placeholder"  dtype ragged_rank value_shape name ))
