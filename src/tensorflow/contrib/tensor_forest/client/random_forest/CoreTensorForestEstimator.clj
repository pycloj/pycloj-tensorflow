(ns tensorflow.contrib.tensor-forest.client.random-forest.CoreTensorForestEstimator
...
(defn CoreTensorForestEstimator 
  "A CORE estimator that can train and evaluate a random forest.
  ...
  ```
  "
  [params device_assigner model_dir feature_columns & {:keys [graph_builder_class config weight_column keys_column feature_engineering_fn early_stopping_rounds early_stopping_loss_threshold num_trainers trainer_id report_feature_importances local_eval version head include_all_in_serving]
                       :or {config None weight_column None keys_column None feature_engineering_fn None version None head None}} ]
    (py/call-attr-kw random-forest "CoreTensorForestEstimator" [params device_assigner model_dir feature_columns] {:graph_builder_class graph_builder_class :config config :weight_column weight_column :keys_column keys_column :feature_engineering_fn feature_engineering_fn :early_stopping_rounds early_stopping_rounds :early_stopping_loss_threshold early_stopping_loss_threshold :num_trainers num_trainers :trainer_id trainer_id :report_feature_importances report_feature_importances :local_eval local_eval :version version :head head :include_all_in_serving include_all_in_serving }))
