(ns tensorflow.contrib.seq2seq.LuongMonotonicAttention
...
(defn LuongMonotonicAttention 
  ...
  ICML 2017.  https://arxiv.org/abs/1704.00784
  "
  [num_units memory memory_sequence_length & {:keys [scale score_mask_value sigmoid_noise sigmoid_noise_seed score_bias_init mode dtype name]
                       :or {score_mask_value None sigmoid_noise_seed None dtype None}} ]
    (py/call-attr-kw seq2seq "LuongMonotonicAttention" [num_units memory memory_sequence_length] {:scale scale :score_mask_value score_mask_value :sigmoid_noise sigmoid_noise :sigmoid_noise_seed sigmoid_noise_seed :score_bias_init score_bias_init :mode mode :dtype dtype :name name }))
