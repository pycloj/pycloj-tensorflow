(ns tensorflow.contrib.cudnn-rnn.CudnnRNNRelu
...
(defn CudnnRNNRelu 
  "Cudnn implementation of the RNN-relu layer."
  [num_layers num_units & {:keys [input_mode direction dropout seed dtype kernel_initializer bias_initializer name]
                       :or {seed None kernel_initializer None bias_initializer None name None}} ]
    (py/call-attr-kw cudnn-rnn "CudnnRNNRelu" [num_layers num_units] {:input_mode input_mode :direction direction :dropout dropout :seed seed :dtype dtype :kernel_initializer kernel_initializer :bias_initializer bias_initializer :name name }))
...
(defn call 
  "Runs the forward step for the RNN model.
    ...
    "
  [self inputs initial_state sequence_lengths  & {:keys [time_major training]} ]
    (py/call-attr-kw self "call" [inputs initial_state sequence_lengths] {:time_major time_major :training training }))

(defn canonical-bias-shapes 
  "Shapes of Cudnn canonical bias tensors."
  [ self ]
    (py/call-attr self "canonical_bias_shapes"))

(defn canonical-weight-shapes 
  "Shapes of Cudnn canonical weight tensors."
  [ self ]
    (py/call-attr self "canonical_weight_shapes"))
...
(defn num-dirs 
  ""
  [ self ]
    (py/call-attr self "num_dirs"))

(defn num-layers 
  ""
  [ self ]
    (py/call-attr self "num_layers"))
...
(defn rnn-mode 
  "Type of RNN cell used.
    ...
    "
  [ self ]
    (py/call-attr self "rnn_mode"))

(defn saveable 
  ""
  [ self ]
    (py/call-attr self "saveable"))
...
(defn state-shape 
  "Shape of the state of Cudnn RNN cells w/o.
    ...
    "
  [ self batch_size ]
  (py/call-attr self "state_shape"  self batch_size ))
