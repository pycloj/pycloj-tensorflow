(ns tensorflow.contrib.kernel-methods.KernelLinearClassifier
...
(defn KernelLinearClassifier 
  ...
  * for each `column` in `feature_columns`:
    ...
    - if `column` is a `RealValuedColumn`, a feature with `key=column.name`
      whose `value` is a `Tensor`.
  "
  [feature_columns model_dir & {:keys [n_classes weight_column_name optimizer kernel_mappers config]
                       :or {weight_column_name None optimizer None kernel_mappers None config None}} ]
    (py/call-attr-kw kernel-methods "KernelLinearClassifier" [feature_columns model_dir] {:n_classes n_classes :weight_column_name weight_column_name :optimizer optimizer :kernel_mappers kernel_mappers :config config }))
...
(defn predict-classes 
  "Runs inference to determine the predicted class per instance.
    ...
    "
  [ self input_fn ]
  (py/call-attr self "predict_classes"  self input_fn ))
...
(defn predict-proba 
  "Runs inference to determine the class probability predictions.
    ...
    "
  [ self input_fn ]
  (py/call-attr self "predict_proba"  self input_fn ))
