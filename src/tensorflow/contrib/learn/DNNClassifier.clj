(ns tensorflow.contrib.learn.DNNClassifier
...
(defn DNNClassifier 
  ...
  * for each `column` in `feature_columns`:
    ...
    - if `column` is a `RealValuedColumn`, a feature with `key=column.name`
      whose `value` is a `Tensor`.
  "
  [hidden_units feature_columns model_dir & {:keys [n_classes weight_column_name optimizer activation_fn dropout gradient_clip_norm enable_centered_bias config feature_engineering_fn embedding_lr_multipliers input_layer_min_slice_size label_keys]
                       :or {weight_column_name None optimizer None dropout None gradient_clip_norm None config None feature_engineering_fn None embedding_lr_multipliers None input_layer_min_slice_size None label_keys None}} ]
    (py/call-attr-kw learn "DNNClassifier" [hidden_units feature_columns model_dir] {:n_classes n_classes :weight_column_name weight_column_name :optimizer optimizer :activation_fn activation_fn :dropout dropout :gradient_clip_norm gradient_clip_norm :enable_centered_bias enable_centered_bias :config config :feature_engineering_fn feature_engineering_fn :embedding_lr_multipliers embedding_lr_multipliers :input_layer_min_slice_size input_layer_min_slice_size :label_keys label_keys }))
