(ns tensorflow.contrib.learn.DNNRegressor
...
(defn DNNRegressor 
  ...
  * for each `column` in `feature_columns`:
    ...
    - if `column` is a `RealValuedColumn`, a feature with `key=column.name`
      whose `value` is a `Tensor`.
  "
  [hidden_units feature_columns model_dir weight_column_name optimizer & {:keys [activation_fn dropout gradient_clip_norm enable_centered_bias config feature_engineering_fn label_dimension embedding_lr_multipliers input_layer_min_slice_size]
                       :or {dropout None gradient_clip_norm None config None feature_engineering_fn None embedding_lr_multipliers None input_layer_min_slice_size None}} ]
    (py/call-attr-kw learn "DNNRegressor" [hidden_units feature_columns model_dir weight_column_name optimizer] {:activation_fn activation_fn :dropout dropout :gradient_clip_norm gradient_clip_norm :enable_centered_bias enable_centered_bias :config config :feature_engineering_fn feature_engineering_fn :label_dimension label_dimension :embedding_lr_multipliers embedding_lr_multipliers :input_layer_min_slice_size input_layer_min_slice_size }))
