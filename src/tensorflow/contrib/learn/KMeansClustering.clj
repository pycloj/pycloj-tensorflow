(ns tensorflow.contrib.learn.KMeansClustering
...
(defn KMeansClustering 
  ...
  for general migration instructions.
  "
  [num_clusters model_dir & {:keys [initial_clusters distance_metric random_seed use_mini_batch mini_batch_steps_per_iteration kmeans_plus_plus_num_retries relative_tolerance config]
                       :or {relative_tolerance None config None}} ]
    (py/call-attr-kw learn "KMeansClustering" [num_clusters model_dir] {:initial_clusters initial_clusters :distance_metric distance_metric :random_seed random_seed :use_mini_batch use_mini_batch :mini_batch_steps_per_iteration mini_batch_steps_per_iteration :kmeans_plus_plus_num_retries kmeans_plus_plus_num_retries :relative_tolerance relative_tolerance :config config }))
...
Please use tf.contrib.factorization.KMeansClustering instead of tf.contrib.learn.KMeansClustering. It has a similar interface, but uses the tf.estimator.Estimator API instead of tf.contrib.learn.Estimator."
  [ self input_fn ]
  (py/call-attr self "predict_cluster_idx"  self input_fn ))
...
Returns:
  ...
  [ self input_fn steps ]
  (py/call-attr self "score"  self input_fn steps ))
...
Returns:
  ...
  [self input_fn  & {:keys [as_iterable]} ]
    (py/call-attr-kw self "transform" [input_fn] {:as_iterable as_iterable }))
