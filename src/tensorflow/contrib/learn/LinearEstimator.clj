(ns tensorflow.contrib.learn.LinearEstimator
...
(defn LinearEstimator 
  ...
  * for column in `feature_columns`:
    ...
    - if isinstance(column, `RealValuedColumn`):
        key=column.name, value=a `Tensor`
  "
  [feature_columns head model_dir weight_column_name optimizer gradient_clip_norm & {:keys [_joint_weights config feature_engineering_fn]
                       :or {config None feature_engineering_fn None}} ]
    (py/call-attr-kw learn "LinearEstimator" [feature_columns head model_dir weight_column_name optimizer gradient_clip_norm] {:_joint_weights _joint_weights :config config :feature_engineering_fn feature_engineering_fn }))
