(ns tensorflow.contrib.learn.DNNEstimator
...
(defn DNNEstimator 
  ...
  * for each `column` in `feature_columns`:
    ...
    - if `column` is a `RealValuedColumn`, a feature with `key=column.name`
      whose `value` is a `Tensor`.
  "
  [head hidden_units feature_columns model_dir optimizer & {:keys [activation_fn dropout gradient_clip_norm config feature_engineering_fn embedding_lr_multipliers input_layer_min_slice_size]
    ...
    (py/call-attr-kw learn "DNNEstimator" [head hidden_units feature_columns model_dir optimizer] {:activation_fn activation_fn :dropout dropout :gradient_clip_norm gradient_clip_norm :config config :feature_engineering_fn feature_engineering_fn :embedding_lr_multipliers embedding_lr_multipliers :input_layer_min_slice_size input_layer_min_slice_size }))
