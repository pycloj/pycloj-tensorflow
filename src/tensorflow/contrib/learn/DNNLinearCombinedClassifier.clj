(ns tensorflow.contrib.learn.DNNLinearCombinedClassifier
...
(defn DNNLinearCombinedClassifier 
  "A classifier for TensorFlow Linear and DNN joined training models.
  ...
  * for each `column` in `dnn_feature_columns` + `linear_feature_columns`:
      ...
      - if `column` is a `RealValuedColumn, a feature with `key=column.name`
        whose `value` is a `Tensor`.
  "
  [model_dir & {:keys [n_classes weight_column_name linear_feature_columns linear_optimizer _joint_linear_weights dnn_feature_columns dnn_optimizer dnn_hidden_units dnn_activation_fn dnn_dropout gradient_clip_norm enable_centered_bias config feature_engineering_fn embedding_lr_multipliers input_layer_min_slice_size label_keys fix_global_step_increment_bug]
                       :or {weight_column_name None linear_feature_columns None linear_optimizer None dnn_feature_columns None dnn_optimizer None dnn_hidden_units None dnn_dropout None gradient_clip_norm None config None feature_engineering_fn None embedding_lr_multipliers None input_layer_min_slice_size None label_keys None}} ]
    (py/call-attr-kw learn "DNNLinearCombinedClassifier" [model_dir] {:n_classes n_classes :weight_column_name weight_column_name :linear_feature_columns linear_feature_columns :linear_optimizer linear_optimizer :_joint_linear_weights _joint_linear_weights :dnn_feature_columns dnn_feature_columns :dnn_optimizer dnn_optimizer :dnn_hidden_units dnn_hidden_units :dnn_activation_fn dnn_activation_fn :dnn_dropout dnn_dropout :gradient_clip_norm gradient_clip_norm :enable_centered_bias enable_centered_bias :config config :feature_engineering_fn feature_engineering_fn :embedding_lr_multipliers embedding_lr_multipliers :input_layer_min_slice_size input_layer_min_slice_size :label_keys label_keys :fix_global_step_increment_bug fix_global_step_increment_bug }))
