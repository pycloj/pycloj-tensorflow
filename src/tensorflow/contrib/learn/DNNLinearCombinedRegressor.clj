(ns tensorflow.contrib.learn.DNNLinearCombinedRegressor
...
(defn DNNLinearCombinedRegressor 
  "A regressor for TensorFlow Linear and DNN joined training models.
  ...
  Input of `fit`, `train`, and `evaluate` should have following features,
    otherwise there will be a `KeyError`:
      ...
      - if `column` is a `RealValuedColumn, a feature with `key=column.name`
        whose `value` is a `Tensor`.
  "
  [model_dir weight_column_name linear_feature_columns linear_optimizer & {:keys [_joint_linear_weights dnn_feature_columns dnn_optimizer dnn_hidden_units dnn_activation_fn dnn_dropout gradient_clip_norm enable_centered_bias label_dimension config feature_engineering_fn embedding_lr_multipliers input_layer_min_slice_size fix_global_step_increment_bug]
                       :or {dnn_feature_columns None dnn_optimizer None dnn_hidden_units None dnn_dropout None gradient_clip_norm None config None feature_engineering_fn None embedding_lr_multipliers None input_layer_min_slice_size None}} ]
    (py/call-attr-kw learn "DNNLinearCombinedRegressor" [model_dir weight_column_name linear_feature_columns linear_optimizer] {:_joint_linear_weights _joint_linear_weights :dnn_feature_columns dnn_feature_columns :dnn_optimizer dnn_optimizer :dnn_hidden_units dnn_hidden_units :dnn_activation_fn dnn_activation_fn :dnn_dropout dnn_dropout :gradient_clip_norm gradient_clip_norm :enable_centered_bias enable_centered_bias :label_dimension label_dimension :config config :feature_engineering_fn feature_engineering_fn :embedding_lr_multipliers embedding_lr_multipliers :input_layer_min_slice_size input_layer_min_slice_size :fix_global_step_increment_bug fix_global_step_increment_bug }))
