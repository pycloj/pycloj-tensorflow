(ns tensorflow.contrib.bayesflow.python.ops.monte-carlo
...
(py/initialize!)
(defonce monte-carlo (import-module "tensorflow.contrib.bayesflow.python.ops.monte_carlo"))
...
Raises:
  ...
  ValueError: if `use_reparametrization=False` and `log_prob` is not a Python
    `callable`."
  [f samples log_prob & {:keys [use_reparametrization axis keep_dims name]
                       :or {name None}} ]
    (py/call-attr-kw monte-carlo "expectation" [f samples log_prob] {:use_reparametrization use_reparametrization :axis axis :keep_dims keep_dims :name name }))
(defn expectation-importance-sampler 
  ...
  "
  [f log_p sampling_dist_q z n seed  & {:keys [name]} ]
    (py/call-attr-kw monte-carlo "expectation_importance_sampler" [f log_p sampling_dist_q z n seed] {:name name }))
(defn expectation-importance-sampler-logspace 
  ...
  Returns:
    Logarithm of the importance sampling estimate.  `Tensor` with `shape` equal
      to batch shape of `q`, and `dtype` = `q.dtype`.
  "
  [log_f log_p sampling_dist_q z n seed  & {:keys [name]} ]
    (py/call-attr-kw monte-carlo "expectation_importance_sampler_logspace" [log_f log_p sampling_dist_q z n seed] {:name name }))
