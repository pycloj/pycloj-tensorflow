(ns tensorflow.contrib.distribute.MirroredStrategy
...
(defn MirroredStrategy 
  ...
  * **Worker**: A `worker` is a TensorFlow `task` that usually maps to one
  ...
  Args:
    ...
    cross_tower_ops: Deprecated alias for `cross_device_ops`.
  "
  [devices num_gpus num_gpus_per_worker cross_device_ops & {:keys [auto_shard_dataset cross_tower_ops]
                       :or {cross_tower_ops None}} ]
    (py/call-attr-kw distribute "MirroredStrategy" [devices num_gpus num_gpus_per_worker cross_device_ops] {:auto_shard_dataset auto_shard_dataset :cross_tower_ops cross_tower_ops }))
