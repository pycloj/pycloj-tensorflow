(ns tensorflow.contrib.distributions.python.ops.bijectors.Softplus
...
(defn Softplus 
  ...
  * `c = 0` results in a non-bijective transformation and triggers an exception.
    ...
    reduction over the event space.
  "
  [hinge_softness  & {:keys [validate_args name]} ]
    (py/call-attr-kw bijectors "Softplus" [hinge_softness] {:validate_args validate_args :name name }))
...
(defn hinge-softness 
  ""
  [ self ]
    (py/call-attr self "hinge_softness"))
(defn inverse 
