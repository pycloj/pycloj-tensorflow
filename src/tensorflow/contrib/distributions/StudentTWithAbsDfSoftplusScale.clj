(ns tensorflow.contrib.distributions.StudentTWithAbsDfSoftplusScale
...
(defn StudentTWithAbsDfSoftplusScale 
  "StudentT with `df = floor(abs(df))` and `scale = softplus(scale)`."
  [df loc scale  & {:keys [validate_args allow_nan_stats name]} ]
    (py/call-attr-kw distributions "StudentTWithAbsDfSoftplusScale" [df loc scale] {:validate_args validate_args :allow_nan_stats allow_nan_stats :name name }))
