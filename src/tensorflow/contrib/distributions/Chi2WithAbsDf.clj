(ns tensorflow.contrib.distributions.Chi2WithAbsDf
...
(defn Chi2WithAbsDf 
  "Chi2 with parameter transform `df = floor(abs(df))`."
  [df  & {:keys [validate_args allow_nan_stats name]} ]
    (py/call-attr-kw distributions "Chi2WithAbsDf" [df] {:validate_args validate_args :allow_nan_stats allow_nan_stats :name name }))
