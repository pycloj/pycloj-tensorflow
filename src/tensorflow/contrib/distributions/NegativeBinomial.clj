(ns tensorflow.contrib.distributions.NegativeBinomial
...
(defn NegativeBinomial 
  ...
  * `n!` is the factorial of `n`.
  ...
  [total_count logits probs  & {:keys [validate_args allow_nan_stats name]} ]
    (py/call-attr-kw distributions "NegativeBinomial" [total_count logits probs] {:validate_args validate_args :allow_nan_stats allow_nan_stats :name name }))
