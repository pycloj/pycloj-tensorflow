(ns tensorflow.contrib.distributions.Mixture
...
(defn Mixture 
  ...
  "
  [cat components  & {:keys [validate_args allow_nan_stats use_static_graph name]} ]
    (py/call-attr-kw distributions "Mixture" [cat components] {:validate_args validate_args :allow_nan_stats allow_nan_stats :use_static_graph use_static_graph :name name }))
...
(defn entropy-lower-bound 
  "A lower bound on the entropy of this mixture model.
    ...
    "
  [self   & {:keys [name]} ]
    (py/call-attr-kw self "entropy_lower_bound" [] {:name name }))
...
(defn num-components 
  ""
  [ self ]
    (py/call-attr self "num_components"))
