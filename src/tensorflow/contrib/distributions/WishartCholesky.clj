(ns tensorflow.contrib.distributions.WishartCholesky
...
(defn WishartCholesky 
  ...
  * `Gamma_k` is the [multivariate Gamma function](
  ...
  "
  [df scale  & {:keys [cholesky_input_output_matrices validate_args allow_nan_stats name]} ]
    (py/call-attr-kw distributions "WishartCholesky" [df scale] {:cholesky_input_output_matrices cholesky_input_output_matrices :validate_args validate_args :allow_nan_stats allow_nan_stats :name name }))
...
(defn cholesky-input-output-matrices 
  "Boolean indicating if `Tensor` input/outputs are Cholesky factorized."
  [ self ]
    (py/call-attr self "cholesky_input_output_matrices"))
...
(defn log-normalization 
  "Computes the log normalizing constant, log(Z)."
  [self   & {:keys [name]} ]
    (py/call-attr-kw self "log_normalization" [] {:name name }))
(defn log-prob 
...
(defn mean-log-det 
  ...
  [self   & {:keys [name]} ]
    (py/call-attr-kw self "mean_log_det" [] {:name name }))
(defn mode 
  "Mode."
  [self   & {:keys [name]} ]
...
(defn scale 
  ...
  [ self  ]
  (py/call-attr self "scale"  self  ))

(defn scale-operator 
  "Wishart distribution scale matrix as an Linear Operator."
  [ self ]
    (py/call-attr self "scale_operator"))
(defn stddev 
  "Standard deviation.
