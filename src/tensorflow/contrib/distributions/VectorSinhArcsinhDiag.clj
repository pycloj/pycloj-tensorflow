(ns tensorflow.contrib.distributions.VectorSinhArcsinhDiag
...
(defn VectorSinhArcsinhDiag 
  ...
  * Larger (smaller) `tailweight` leads to fatter (thinner) tails.
  ...
  ```
  "
  [loc scale_diag scale_identity_multiplier skewness tailweight distribution  & {:keys [validate_args allow_nan_stats name]} ]
    (py/call-attr-kw distributions "VectorSinhArcsinhDiag" [loc scale_diag scale_identity_multiplier skewness tailweight distribution] {:validate_args validate_args :allow_nan_stats allow_nan_stats :name name }))
