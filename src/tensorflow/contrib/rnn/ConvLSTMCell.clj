(ns tensorflow.contrib.rnn.ConvLSTMCell
...
(defn ConvLSTMCell 
  ...
  https://arxiv.org/pdf/1506.04214v1.pdf
  "
  [conv_ndims input_shape output_channels kernel_shape & {:keys [use_bias skip_connection forget_bias initializers name]
                       :or {initializers None}} ]
    (py/call-attr-kw rnn "ConvLSTMCell" [conv_ndims input_shape output_channels kernel_shape] {:use_bias use_bias :skip_connection skip_connection :forget_bias forget_bias :initializers initializers :name name }))
...
(defn call 
  ""
  [ self inputs state scope ]
  (py/call-attr self "call"  self inputs state scope ))
