(ns tensorflow.contrib.rnn.BidirectionalGridLSTMCell
...
(defn BidirectionalGridLSTMCell 
  "Bidirectional GridLstm cell.
  ...
  The current implementation uses different weights for the two directions.
  "
  [num_units & {:keys [use_peepholes share_time_frequency_weights cell_clip initializer num_unit_shards forget_bias feature_size frequency_skip num_frequency_blocks start_freqindex_list end_freqindex_list couple_input_forget_gates backward_slice_offset reuse]
                       :or {cell_clip None initializer None feature_size None frequency_skip None num_frequency_blocks None start_freqindex_list None end_freqindex_list None reuse None}} ]
    (py/call-attr-kw rnn "BidirectionalGridLSTMCell" [num_units] {:use_peepholes use_peepholes :share_time_frequency_weights share_time_frequency_weights :cell_clip cell_clip :initializer initializer :num_unit_shards num_unit_shards :forget_bias forget_bias :feature_size feature_size :frequency_skip frequency_skip :num_frequency_blocks num_frequency_blocks :start_freqindex_list start_freqindex_list :end_freqindex_list end_freqindex_list :couple_input_forget_gates couple_input_forget_gates :backward_slice_offset backward_slice_offset :reuse reuse }))
...
(defn build 
  ""
  [ self _ ]
  (py/call-attr self "build"  self _ ))
...
(defn call 
  "Run one step of LSTM.
    ...
    "
  [ self inputs state ]
  (py/call-attr self "call"  self inputs state ))
...
(defn state-tuple-type 
  ""
  [ self ]
    (py/call-attr self "state_tuple_type"))
...
(defn zero-state 
  ...
  (py/call-attr self "zero_state"  self batch_size dtype ))
