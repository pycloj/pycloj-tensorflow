...
(defn Dropout 
  ...
  Arguments:
    ...
    name: The name of the layer (string).
  "
  [ & {:keys [rate noise_shape seed name]
       :or {noise_shape None seed None name None}} ]
  
   (py/call-attr-kw layers "Dropout" [] {:rate rate :noise_shape noise_shape :seed seed :name name }))
...
(defn call 
  ...
  [self inputs  & {:keys [training]} ]
    (py/call-attr-kw self "call" [inputs] {:training training }))
