(ns tensorflow.python.keras.api.-v1.keras.layers.AbstractRNNCell
...
(defn AbstractRNNCell 
  ...
  for each `s` in `self.batch_size`.
  "
  [ & {:keys [trainable name dtype dynamic]
       :or {name None dtype None}} ]
  
   (py/call-attr-kw layers "AbstractRNNCell" [] {:trainable trainable :name name :dtype dtype :dynamic dynamic }))
...
(defn call 
  "The function that contains the logic for one RNN step calculation.
    ...
    "
  [ self inputs states ]
  (py/call-attr self "call"  self inputs states ))
