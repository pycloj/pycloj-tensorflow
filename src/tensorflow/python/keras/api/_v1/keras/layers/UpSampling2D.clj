(ns tensorflow.python.keras.api.-v1.keras.layers.UpSampling2D
...
(defn UpSampling2D 
  ...
  Output shape:
    ...
    - If `data_format` is `\"channels_first\"`:
        `(batch, channels, upsampled_rows, upsampled_cols)`
  "
  [ & {:keys [size data_format interpolation]
       :or {data_format None}} ]
  
   (py/call-attr-kw layers "UpSampling2D" [] {:size size :data_format data_format :interpolation interpolation }))
