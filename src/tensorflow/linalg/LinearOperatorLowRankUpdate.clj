(ns tensorflow.linalg.LinearOperatorLowRankUpdate
...
(defn LinearOperatorLowRankUpdate 
  "Perturb a `LinearOperator` with a rank `K` update.
  ...
  * If `is_X == None` (the default), callers should have no expectation either
    way.
  "
  [base_operator u diag_update v is_diag_update_positive is_non_singular is_self_adjoint is_positive_definite is_square  & {:keys [name]} ]
    (py/call-attr-kw linalg "LinearOperatorLowRankUpdate" [base_operator u diag_update v is_diag_update_positive is_non_singular is_self_adjoint is_positive_definite is_square] {:name name }))
...
(defn base-operator 
  ...
  [ self ]
    (py/call-attr self "base_operator"))
...
(defn diag-operator 
  ...
  [ self ]
    (py/call-attr self "diag_operator"))
(defn diag-part 
...
(defn is-diag-update-positive 
  "If this operator is `A = L + U D V^H`, this hints `D > 0` elementwise."
  [ self ]
    (py/call-attr self "is_diag_update_positive"))
