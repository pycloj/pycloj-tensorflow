(ns tensorflow.linalg.LinearOperatorCirculant2D
...
(py/initialize!)
(defonce linalg (import-module "tensorflow.linalg"))

(defn LinearOperatorCirculant2D 
  "`LinearOperator` acting like a block circulant matrix.
  ...
  * If `is_X == None` (the default), callers should have no expectation either
    way.
  "
  [spectrum & {:keys [input_output_dtype is_non_singular is_self_adjoint is_positive_definite is_square name]
                       :or {is_non_singular None is_self_adjoint None is_positive_definite None}} ]
    (py/call-attr-kw linalg "LinearOperatorCirculant2D" [spectrum] {:input_output_dtype input_output_dtype :is_non_singular is_non_singular :is_self_adjoint is_self_adjoint :is_positive_definite is_positive_definite :is_square is_square :name name }))
...
(defn assert-hermitian-spectrum 
  "Returns an `Op` that asserts this operator has Hermitian spectrum.
    ...
    "
  [self   & {:keys [name]} ]
    (py/call-attr-kw self "assert_hermitian_spectrum" [] {:name name }))
...
(defn block-depth 
  "Depth of recursively defined circulant blocks defining this `Operator`.
    ...
    "
  [ self ]
    (py/call-attr self "block_depth"))

(defn block-shape 
  ""
  [ self ]
    (py/call-attr self "block_shape"))

(defn block-shape-tensor 
  "Shape of the block dimensions of `self.spectrum`."
  [ self  ]
  (py/call-attr self "block_shape_tensor"  self  ))
(defn cholesky 
...
(defn convolution-kernel 
  "Convolution kernel corresponding to `self.spectrum`.
    ...
    "
  [self   & {:keys [name]} ]
    (py/call-attr-kw self "convolution_kernel" [] {:name name }))
(defn determinant 
