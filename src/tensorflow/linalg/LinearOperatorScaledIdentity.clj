(ns tensorflow.linalg.LinearOperatorScaledIdentity
...
(defn LinearOperatorScaledIdentity 
  "`LinearOperator` acting like a scaled [batch] identity matrix `A = c I`.
  ...
  * If `is_X == None` (the default), callers should have no expectation either
    way.
  "
  [num_rows multiplier is_non_singular is_self_adjoint is_positive_definite  & {:keys [is_square assert_proper_shapes name]} ]
    (py/call-attr-kw linalg "LinearOperatorScaledIdentity" [num_rows multiplier is_non_singular is_self_adjoint is_positive_definite] {:is_square is_square :assert_proper_shapes assert_proper_shapes :name name }))
