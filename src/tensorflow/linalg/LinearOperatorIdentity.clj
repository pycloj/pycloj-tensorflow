(ns tensorflow.linalg.LinearOperatorIdentity
...
(defn LinearOperatorIdentity 
  ...
  * If `is_X == None` (the default), callers should have no expectation either
    way.
  "
  [num_rows batch_shape dtype  & {:keys [is_non_singular is_self_adjoint is_positive_definite is_square assert_proper_shapes name]} ]
    (py/call-attr-kw linalg "LinearOperatorIdentity" [num_rows batch_shape dtype] {:is_non_singular is_non_singular :is_self_adjoint is_self_adjoint :is_positive_definite is_positive_definite :is_square is_square :assert_proper_shapes assert_proper_shapes :name name }))
