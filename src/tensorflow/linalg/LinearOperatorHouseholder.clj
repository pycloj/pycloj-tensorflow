(ns tensorflow.linalg.LinearOperatorHouseholder
...
(defn LinearOperatorHouseholder 
  "`LinearOperator` acting like a [batch] of Householder transformations.
  ...
  * If `is_X == None` (the default), callers should have no expectation either
    way.
  "
  [reflection_axis is_non_singular is_self_adjoint is_positive_definite is_square  & {:keys [name]} ]
    (py/call-attr-kw linalg "LinearOperatorHouseholder" [reflection_axis is_non_singular is_self_adjoint is_positive_definite is_square] {:name name }))
...
(defn reflection-axis 
  ""
  [ self ]
    (py/call-attr self "reflection_axis"))
