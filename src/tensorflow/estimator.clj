(ns tensorflow-estimator.python.estimator.api.-v1.estimator
...
(defn classifier-parse-example-spec 
  ...
  * It is difficult to map expected label by a classifier such as
  ...
  "
  [feature_columns label_key & {:keys [label_dtype label_default weight_column]
                       :or {label_default None weight_column None}} ]
    (py/call-attr-kw estimator "classifier_parse_example_spec" [feature_columns label_key] {:label_dtype label_dtype :label_default label_default :weight_column weight_column }))

(defn regressor-parse-example-spec 
  ...
  * It is difficult to map expected label by a regressor such as `DNNRegressor`
  ...
  Raises:
    ...
    ValueError: if label_key is None.
  "
  [feature_columns label_key & {:keys [label_dtype label_default label_dimension weight_column]
                       :or {label_default None weight_column None}} ]
    (py/call-attr-kw estimator "regressor_parse_example_spec" [feature_columns label_key] {:label_dtype label_dtype :label_default label_default :label_dimension label_dimension :weight_column weight_column }))
...
(defn train-and-evaluate 
  ...
  (py/call-attr estimator "train_and_evaluate"  estimator train_spec eval_spec ))
