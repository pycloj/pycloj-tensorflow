(ns tensorflow-estimator.python.estimator.api.-v1.estimator.DNNLinearCombinedClassifier
...
(defn DNNLinearCombinedClassifier 
  ...
  * for each `column` in `dnn_feature_columns` + `linear_feature_columns`:
  ...
  @end_compatibility
  "
  [model_dir linear_feature_columns & {:keys [linear_optimizer dnn_feature_columns dnn_optimizer dnn_hidden_units dnn_activation_fn dnn_dropout n_classes weight_column label_vocabulary input_layer_partitioner config warm_start_from loss_reduction batch_norm linear_sparse_combiner]
                       :or {dnn_feature_columns None dnn_hidden_units None dnn_dropout None weight_column None label_vocabulary None input_layer_partitioner None config None warm_start_from None}} ]
    (py/call-attr-kw estimator "DNNLinearCombinedClassifier" [model_dir linear_feature_columns] {:linear_optimizer linear_optimizer :dnn_feature_columns dnn_feature_columns :dnn_optimizer dnn_optimizer :dnn_hidden_units dnn_hidden_units :dnn_activation_fn dnn_activation_fn :dnn_dropout dnn_dropout :n_classes n_classes :weight_column weight_column :label_vocabulary label_vocabulary :input_layer_partitioner input_layer_partitioner :config config :warm_start_from warm_start_from :loss_reduction loss_reduction :batch_norm batch_norm :linear_sparse_combiner linear_sparse_combiner }))
