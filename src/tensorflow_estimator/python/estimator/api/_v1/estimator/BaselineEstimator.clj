(ns tensorflow-estimator.python.estimator.api.-v1.estimator.BaselineEstimator
...
(defn BaselineEstimator 
  ...
  * if `weight_column` is specified in the `head` constructor (and not None) for
    ...
    `key=weight_column` whose value is a `Tensor`.
  "
  [head model_dir & {:keys [optimizer config]
                       :or {config None}} ]
    (py/call-attr-kw estimator "BaselineEstimator" [head model_dir] {:optimizer optimizer :config config }))
