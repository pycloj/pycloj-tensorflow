(ns tensorflow-estimator.python.estimator.api.-v1.estimator.BaselineRegressor
...
(defn BaselineRegressor 
  ...
  * if `weight_column` is not `None`, a feature with
  ...
  @end_compatibility
  "
  [model_dir & {:keys [label_dimension weight_column optimizer config loss_reduction]
                       :or {weight_column None config None}} ]
    (py/call-attr-kw estimator "BaselineRegressor" [model_dir] {:label_dimension label_dimension :weight_column weight_column :optimizer optimizer :config config :loss_reduction loss_reduction }))
