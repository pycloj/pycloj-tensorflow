(ns tensorflow-estimator.python.estimator.api.-v1.estimator.LinearRegressor
...
(defn LinearRegressor 
  ...
  * for each `column` in `feature_columns`:
  ...
  @end_compatibility
  "
  [feature_columns model_dir & {:keys [label_dimension weight_column optimizer config partitioner warm_start_from loss_reduction sparse_combiner]
                       :or {weight_column None config None partitioner None warm_start_from None}} ]
    (py/call-attr-kw estimator "LinearRegressor" [feature_columns model_dir] {:label_dimension label_dimension :weight_column weight_column :optimizer optimizer :config config :partitioner partitioner :warm_start_from warm_start_from :loss_reduction loss_reduction :sparse_combiner sparse_combiner }))
