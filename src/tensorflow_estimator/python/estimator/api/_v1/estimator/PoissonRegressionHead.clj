(ns tensorflow-estimator.python.estimator.api.-v1.estimator.PoissonRegressionHead
...
(defn PoissonRegressionHead 
  ...
  Args:
    ...
    name: name of the head. If provided, summary and metrics keys will be
      suffixed by `\"/\" + name`. Also used as `name_scope` when creating ops.
  "
  [ & {:keys [label_dimension weight_column loss_reduction compute_full_loss name]
       :or {weight_column None name None}} ]
  
   (py/call-attr-kw estimator "PoissonRegressionHead" [] {:label_dimension label_dimension :weight_column weight_column :loss_reduction loss_reduction :compute_full_loss compute_full_loss :name name }))
