(ns tensorflow-estimator.python.estimator.api.-v1.estimator.DNNRegressor
...
(defn DNNRegressor 
  ...
  * for each `column` in `feature_columns`:
  ...
  @end_compatibility
  "
  [hidden_units feature_columns model_dir & {:keys [label_dimension weight_column optimizer activation_fn dropout input_layer_partitioner config warm_start_from loss_reduction batch_norm]
                       :or {weight_column None dropout None input_layer_partitioner None config None warm_start_from None}} ]
    (py/call-attr-kw estimator "DNNRegressor" [hidden_units feature_columns model_dir] {:label_dimension label_dimension :weight_column weight_column :optimizer optimizer :activation_fn activation_fn :dropout dropout :input_layer_partitioner input_layer_partitioner :config config :warm_start_from warm_start_from :loss_reduction loss_reduction :batch_norm batch_norm }))
