(ns tensorflow-estimator.python.estimator.api.-v1.estimator.LinearEstimator
...
(defn LinearEstimator 
  ...
  * for each `column` in `feature_columns`:
  ...
  @end_compatibility
  "
  [head feature_columns model_dir & {:keys [optimizer config partitioner sparse_combiner]
                       :or {config None partitioner None}} ]
    (py/call-attr-kw estimator "LinearEstimator" [head feature_columns model_dir] {:optimizer optimizer :config config :partitioner partitioner :sparse_combiner sparse_combiner }))
