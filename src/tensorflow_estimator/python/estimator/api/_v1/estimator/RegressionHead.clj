(ns tensorflow-estimator.python.estimator.api.-v1.estimator.RegressionHead
...
(defn RegressionHead 
  ...
  Args:
    ...
    name: name of the head. If provided, summary and metrics keys will be
      suffixed by `\"/\" + name`. Also used as `name_scope` when creating ops.
  "
  [ & {:keys [label_dimension weight_column loss_reduction loss_fn inverse_link_fn name]
       :or {weight_column None loss_fn None inverse_link_fn None name None}} ]
  
   (py/call-attr-kw estimator "RegressionHead" [] {:label_dimension label_dimension :weight_column weight_column :loss_reduction loss_reduction :loss_fn loss_fn :inverse_link_fn inverse_link_fn :name name }))
