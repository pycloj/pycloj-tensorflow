(ns tensorflow-estimator.python.estimator.api.-v1.estimator.BaselineClassifier
...
(defn BaselineClassifier 
  ...
  * if `weight_column` is not `None`, a feature with
  ...
  @end_compatibility
  "
  [model_dir & {:keys [n_classes weight_column label_vocabulary optimizer config loss_reduction]
                       :or {weight_column None label_vocabulary None config None}} ]
    (py/call-attr-kw estimator "BaselineClassifier" [model_dir] {:n_classes n_classes :weight_column weight_column :label_vocabulary label_vocabulary :optimizer optimizer :config config :loss_reduction loss_reduction }))
